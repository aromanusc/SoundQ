...
class SeldModel(torch.nn.Module):
    ...
    def forward(self, x, bbox):
        ...
        # Video embedding
        bbox = bbox.transpose(1, 2).contiguous()
        x_v = bbox.reshape(-1, 6*82)
        x_v = self.vision_encoder(x_v)
        x_v = x_v.reshape(x.shape[0], x.shape[1], -1)
        
        # Concatenate audio embedding x and video embedding x_v
        x = torch.cat((x, x_v), axis=2)

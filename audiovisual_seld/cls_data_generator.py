        ...
        if self._is_eval:
            for i in range(self._nb_total_batches):
                ...
                while len(self._circ_buf_feat) < self._feature_batch_seq_len:
                    ...
                    bbox_dir = "/scratch/ssd1/seld-feats-full/bbox_dev"
                    bbox_path = os.path.join(bbox_dir, self._filenames_list[file_cnt])
                    bbox_feat = []
                    if os.path.exists(bbox_path):
                        bbox_feat = np.load(bbox_path)
                    else:
                        # continue
                        tmp_shape = np.reshape(temp_feat, (-1, self._nb_mel_bins * self._nb_ch)).shape
                        bbox_feat = np.ones((tmp_shape[0]*3, 6, 82))*0.0

                    max_frame_idx = int(len(bbox_feat)//3)
                    temp_feat = temp_feat[:max_frame_idx]
                    temp_label = temp_label[:max_frame_idx]
                    bbox_feat = bbox_feat[:max_frame_idx*3]
                    for row_cnt, row in enumerate(temp_feat):
                        self._circ_buf_feat.append(row)

                    for index in range(0, len(bbox_feat), 3): # take only idx%3 elements 
                        self._circ_buf_bbox.append(bbox_feat[index])
                    ...
                    if self._per_file:
                        ...
                        extra_bbox = np.ones((extra_frames, bbox_feat.shape[1], bbox_feat.shape[2])) * 1e-6

                        for row_cnt, row in enumerate(extra_feat):
                            self._circ_buf_feat.append(row)

                        for row_cnt, row in enumerate(extra_bbox):
                            self._circ_buf_bbox.append(row)
                ...
                # BBox - read one batch size from the circular buffer
                feat_bbox = np.zeros((self._feature_batch_seq_len, 6, 82))
                for j in range(self._feature_batch_seq_len):
                    feat_bbox[j, :] = self._circ_buf_bbox.popleft()
                feat_bbox = np.reshape(feat_bbox, (self._feature_batch_seq_len, 6, 82))

                # Split to sequences
                feat = self._split_in_seqs(feat, self._feature_seq_len)
                feat = np.transpose(feat, (0, 2, 1, 3))

                # Bbox - Split to sequences
                feat_bbox = self._split_in_seqs(feat_bbox, self._feature_seq_len)
                feat_bbox = np.transpose(feat_bbox, (0, 2, 1, 3))

                yield feat, np.array(feat_bbox)
            ...
            for i in range(self._nb_total_batches):
                ...
                while len(self._circ_buf_feat) < self._feature_batch_seq_len:
                    ...
                    if os.path.exists(bbox_path):
                        bbox_feat = np.load(bbox_path)
                        frames = librosa.util.frame(bbox_feat, frame_length=30, hop_length=6, axis=0)
                    else:
                        # continue
                        tmp_shape = np.reshape(temp_feat, (-1, self._nb_mel_bins * self._nb_ch)).shape
                        bbox_feat = np.ones((tmp_shape[0]*3, 6, 82))*0.0
                    if not self._per_file: 
                        ...
                        bbox_feat = bbox_feat[:temp_mul*self._feature_seq_len*3, :, :] # bbox is sampled at 30FPS, roughly 3x audio frames
                    ...
                    for idx in range(0, len(bbox_feat), 3):
                        self._circ_buf_bbox.append(bbox_feat[idx])
                    ...
                    if self._per_file:
                        ...
                        extra_bbox = np.zeros((feat_extra_frames, bbox_feat.shape[1], bbox_feat.shape[2])) * 1e-6
                        ...
                        for f_row in extra_feat:
                            self._circ_buf_feat.append(f_row)
                        for f_row in extra_bbox:
                            self._circ_buf_bbox.append(f_row)
                ...
                yield feat, label, np.array(feat_bbox)

...
#            pool.join()
    ...
    def extract_all_bboxes(self):
        self.get_frame_stats()
        self._label_dir = "/scratch/ssd1/seld-feats-full/bbox_dev"
        self._transform_needs_flip = ["aug_acs_1", "aug_acs_4", "aug_acs_5", "aug_acs_8"]
        bbox_dir = "/scratch/data/audio-visual-seld-dcase2023/data_dcase2023_task3/video_bboxmat_dev"
        self._n_box = 6
        self._len_dist = 36
        len_class_embedd = 8
        ...
        #     self._aud_dir, self._desc_dir, self._label_dir))
        create_folder(self._label_dir)
        for sub_folder in os.listdir(bbox_dir):
            loc_desc_folder = os.path.join(bbox_dir, sub_folder)
            for file_cnt, file_name in enumerate(os.listdir(loc_desc_folder)):
                bboxes = np.load(os.path.join(loc_desc_folder, file_name))
                flipped = not all(flip_file not in file_name for flip_file in self._transform_needs_flip)
                bbox_file = []
                for frame_bboxes in bboxes:
                    for bbox_in in frame_bboxes:
                        dist_azi = np.zeros((self._n_box, self._len_dist + 1))
                        dist_ele = np.zeros((self._n_box, self._len_dist + 1))
                        class_enc = np.zeros((self._n_box, len_class_embedd))
                        for i, each_box in enumerate(bbox_in[:self._n_box]):
                            if np.sum(each_box) > 0:
                                each_box[0] = each_box[0] / 1920
                                each_box[1] = each_box[1] / 960
                                each_box[2] = each_box[2] / 1920
                                each_box[3] = each_box[3] / 960
                                center_azi = (each_box[0] + each_box[2]) / 2  # 0 - 1
                                # [asroman] - here we need to be super careful with this azimuth since we will be flipping
                                center_ele = (each_box[1] + each_box[3]) / 2 if not flipped else 1 - ((each_box[1] + each_box[3]) / 2)
                                len_azi = each_box[2] - each_box[0]  # 0 - 1
                                len_ele = each_box[3] - each_box[1]
                                # print("concat shape", np.concatenate((self._func_Gauss(points, center_azi * self._len_dist, len_azi / 2 * self._len_dist), class_embed)).shape)
                                class_enc[i] = each_box[4:]
                        ...
                        out_bbox = np.concatenate((dist_azi, dist_ele, class_enc), axis=1) # (6, 82) [n_bbox, azi_embedd + ele_embedd + class_enc (37+37+8)]  
                        bbox_file.append(out_bbox)
                print(np.array(bbox_file).shape)
                np.save(os.path.join(self._label_dir, '{}.npy'.format(file_name.split('.')[0])), np.array(bbox_file)) # [nframes, n_bbox, azi_embedd + ele_embedd + class_enc (37+37+8)] 

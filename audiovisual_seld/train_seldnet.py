...
def test_epoch(data_generator, model, criterion, dcase_output_folder, params, device, remove_visual=False):
    ...
    file_cnt = 0
    with torch.no_grad():
        for data, target, data_bbox in data_generator.generate():
            # load one batch of data
            data, target, data_bbox = torch.tensor(data).to(device).float(), torch.tensor(target).to(device).float(), torch.tensor(data_bbox).to(device).float()

            # process the batch of data based on chosen mode
            if remove_visual:
                # print("Removing visual path")
                output = model(data, torch.zeros_like(data_bbox))
            else:
                output = model(data, data_bbox)
